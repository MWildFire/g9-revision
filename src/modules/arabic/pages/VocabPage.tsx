...
import { VOCAB_THEMES } from '../data/arabicData';
          ...
          <div className="bg-bg-secondary border border-border rounded-md overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-bg-tertiary/30 text-text-secondary text-xs uppercase tracking-wider">
                <tr>
                  <th className="text-right px-4 py-2">العربية</th>
                  <th className="text-left px-4 py-2">Translit.</th>
              ...
              <tbody>
                {theme.items.map((v, i) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-bg-secondary' : 'bg-bg-primary/30'}>
                    <td className="px-4 py-1.5 text-right text-lg" style={{ fontFamily: arFont }} dir="rtl">{v.ar}</td>
                    <td className="px-4 py-1.5 font-mono text-xs">{v.translit}</td>
